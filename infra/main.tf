...
# Default provider (your usual region)
provider "aws" {
  region = var.region
}

# Separate provider in us-east-1 for the CloudFront certificate
provider "aws" {
  alias  = "us_east_1"
  region = "us-east-1"
}
...
# Website content bucket (private, CloudFront-only)
...
# Root domain -> CloudFront (A/AAAA alias)
...
# www subdomain -> CloudFront (CNAME)
